...
async def run_service(settings: Settings) -> None:
    ...
    try:
        ...
        if getattr(settings, "telegram_mode", "polling") == "webhook":
        ...
        tasks_to_watch = {telegram_task}
        if dispatcher_task is not None:
            tasks_to_watch.add(dispatcher_task)
        ...
        if telegram_task in done and not stop_event.is_set():
            raise RuntimeError("Telegram runtime stopped unexpectedly")
    finally:
        if dispatcher is not None:
            dispatcher.stop()
        if stop_event is not None:
            stop_event.set()
        tasks = [task for task in (telegram_task, dispatcher_task) if task is not None]
        for task in tasks:
            if not task.done():
                task.cancel()
        if tasks:
